...
export default function TopNavigation() {
  ...
  return (
    <header className="relative z-30 border-b border-sky-100 bg-white/90 shadow-[0_8px_30px_-24px_rgba(15,76,117,0.65)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-3 sm:px-6 lg:flex-row lg:items-center lg:px-8">
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <NavLink
                ...
                className={`inline-flex min-h-10 flex-none items-center justify-center rounded-full px-3.5 text-xs font-bold transition sm:text-sm ${active ? 'bg-sky-600 text-white shadow-md' : 'bg-sky-50 text-sky-700 hover:bg-sky-100'}`}
              >
